...
pull_single_all_location <- function(start_date,
                                     end_date,
                                     region_type,
                                     region_id,
                                     return_type) {
  ...
  if (length(region_id) > 1) {
    ArgumentCheck::addError(msg = "'region_id' is incorrectly specified. Please use a single character",
                            argcheck = Check)
  }
  ...
  return_data_count <- readr::read_csv(
    glue::glue(
      "http://api.adu.org.za/sabap2/v2/R/{start_date}/{end_date}/{region_type}/{region_id}/count?format=csv"
    ),
  ...
  if (return_data_count > 0) {
    readr::read_csv(
      glue::glue(
        "http://api.adu.org.za/sabap2/v2/R/{start_date}/{end_date}/{region_type}/{region_id}/{return_type}/all?format=csv"
      ),
      col_types = readr::cols()
    )
